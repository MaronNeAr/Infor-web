        ...
        <div class="banner1">
            <div class="certificate">
                ...
                <div class="certificate-icon">
                    <img src="@/assets/img/banner.png" alt="Certificate Icon" />
                </div>
                <div class="certificate-title" style="display: flex;">
                    <div class="certificate-title-1" style="font-size: 80px;z-index:1 !important;">证件</div>
        ...
        <v-row class="card-container">
            ...
            <v-col md="7" cols="12">
                <v-card class="article-wrapper">
                    <!-- 评论 -->
                    <v-btn color="primary float-right" @click="dialog = true" class="upload">上传</v-btn>
            ...
            <v-col md="3" cols="12" class="d-md-block d-none">
                ...
                <div style="position: relative;margin-left:50px;margin-top:10px ;">
                    ...
                    <v-card class="right-container">
                        ...
                        <div class="tag-list">
                            <v-chip class="ma-2" v-for="(count, label) in labelCounts" :key="label" color="blue" text-color="white" @click="change_tag(label)">
    ...
    export default {
        ...
        methods: {
            ...
            async getCards() {
                ...
                await axios.post('api/card', params).then(response => {
                    if (response.data.success) {
                        const result = response.data.data;
                        result.forEach(item => {
                            console.log(item)
                            if (item.type == "idcard-back" || item.type == "idcard-front") {
                                this.category_list[0].card_list.push(item);
                            } else if (item.type == "bankcard") {
                                this.category_list[1].card_list.push(item);
                            } else if (item.type == "hk_macau_taiwan_exitentrypermit") {
                ...
                return this.category_list.filter((category) => {
                    return category.card_list.some((card) => {
                        return (
                            ...
                            new Date(card.date) <= new Date(this.dates[1]))||this.dates.length < 2)&&
